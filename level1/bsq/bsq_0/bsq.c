...
Map *get_map(char *argv)
{
    ...
    map->map = (char **)calloc(map->lines + 1, sizeof(char *));
    if (!map->map)
        return (fclose(file), free(map), NULL);
    map->width = 0;
    ...
    for (int i = 0; i < map->lines; i++)
    {
        ...
        line_len = getline(&line, &buf_len, file);
        if (!line)
            return (fclose(file), free_map(map), NULL);
        if (line[line_len - 1] != '\n')
            return (free(line), fclose(file), free_map(map), NULL);
        line[line_len - 1] = '\0';
        if (i == 0)
